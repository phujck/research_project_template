...
learning, the same geometry appears under different names---shortcut learning 
\cite{geirhos2020shortcut}, underspecification \cite{d'amour2022underspecification}, 
and brittle generalisation under shift---where models achieve high average 
...
budget \cite{sims2003implications}, that cheapness is converted into a posture of 
diminished verification, pushing error into the uncompressed tails.
...
cost---canonical bases, standard approximations, inherited intuitions---because 
...
expensive or absent---no grounded retrieval, no cross-check, no coupling to a 
second representation---cheap internal objects are emitted as if they were safe.
...
\subsection{The mono-representation problem}

The trap is not specific to a particular algorithm, nor even to human psychology. 
It is endemic to any regime in which a single representational lens dominates---any 
system in which ``what it means for something to make sense'' is largely 
exhausted by ``what is cheap in the prevailing code.'' In that regime, 
obviousness becomes self-certifying: internal fluency is mistaken for adequacy, 
and verification becomes mere self-consistency inside the same lens. Tails are 
then defined not only by rarity in the world but by clumsiness in the 
representation: they are the regions the lens itself discourages you from 
inhabiting.

\subsection{Coupling inequivalent representations by mutual consistency}

This suggests a structural antidote: not more cleverness inside the same 
representation, but coupling between inequivalent representations. If obviousness 
is representation-dependent, then a natural replacement for ``verification against 
the world'' is ``survival under translation.'' Different representations make 
different structures cheap and possess different blind spots. By demanding 
mutual consistency---that a claim remain stable when expressed in a genuinely 
different representational basis---one can force scrutiny back into precisely 
the places where any single representation would least like to pay it.

Operationally: maintain an ensemble of structurally distinct representations 
$R^{(1)}, R^{(2)}, \dots$, and treat disagreement under translation as an explicit 
signal that the representational ledger is incomplete---some assumption has been 
smuggled in, some tail regime has not been tested, or some cost has been hidden 
by a convenient basis. This does not eliminate the need for external grounding, 
but it can prevent obviousness in one lens from becoming unquestionable in 
all lenses.
...
epistemological pluralism \cite{feyerabend1975against}, the only rational 
response to this paper is to doubt its coherence.
