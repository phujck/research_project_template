...
Attention is pushed to the margins where work feels ``hard,'' 
leaving the uncompressed corners of the world systematically unverified.

\subsection{Epiplexity as Representational Investment}

To ground the cost signal $C_{cons}$ in modern information theory, we invoke the 
recently introduced concept of \textit{Epiplexity} \cite{finzi2026epiplexity}. 
Classical measures like Kolmogorov complexity or Shannon entropy assume 
observers with infinite computational capacity. By contrast, epiplexity 
formalises the structural content that a \textit{computationally bounded 
observer} can actually extract from data. 

Finzi et al. \cite{finzi2026epiplexity} provide a high-level decomposition of 
the total information available to such an observer under a compute bound $T$:
\begin{equation}
    \mathcal{E}_T(D) = L(M) + H_T(D | M)
\end{equation}
where $L(M)$ is the length of the model or program (the representational 
investment) and $H_T(D | M)$ is the \textit{time-bounded entropy} or residual 
codelength---the portion of the data that remains unpredictable noise to that 
specific observer. 

In our framework, we identify $C_{cons}(P|R)$ as the local, 
representation-conditioned construction cost experienced by the agent. 
Representational progress $R_0 \to R_1$ is an investment that increases the 
extractable structure $L(M)$ for the observer, effectively moving information 
mass from the residual $H_T$ into the model $M$. This shift lowers the marginal 
cost $C_{cons}$ for a structured subset of claims $\mathcal{P}_{fluent} \subset \mathcal{P}$, 
making them feel ``obvious.''
...
\subsection{Epiplexity and the Obviousness Trap}

Using the lens of epiplexity, we can restate the mechanism with formal 
precision. The ``structural investment'' in a representation---the bits 
dedicated to the model $L(M)$---is what manufactures fluency. By adopting a 
more powerful program or basis, an agent increases the amount of extractable 
structure. However, under finite verification budgets, that very fluency 
becomes a stop-signal: the agent rationally (but fragiley) divests from verifying 
what is cheap to construct. The result is a characteristic error geometry: 
error is not eliminated, but relocated to the high-epiplexity tails where 
the representation does not compress.

This dynamic provides a clean handle on \textit{hallucination}. A 
hallucination is not random noise; it is a low-cost internal object emitted 
without paying the additional ``coupling cost'' of external verification. In a 
generative model, coherent completions are cheap under the learned program $M$. 
What is expensive is fidelity to the contingent, high-friction constraints of 
the world. Under a budget rule that treats cheapness as a proxy for safety, 
these expensive constraints are the first to be dropped. Hallucination is the 
natural output of an obviousness engine run without a verification channel.

The \textit{mono-representation problem} follows as a consequence of single 
program class dominance. When one representation defines what counts as 
efficient, obviousness becomes self-certifying. Scrutiny is applied only 
internally, as a check for self-consistency within the prevailing code. To 
escape this, we propose the structural antidote of \textit{coupling inequivalent 
representations}. By demanding \textit{mutual consistency} across structurally 
distinct program classes, an agent can force scrutiny back into the precise 
regions where any single compression would least like to pay it.
