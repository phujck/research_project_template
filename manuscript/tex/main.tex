...
\begin{abstract}
We propose a formal theory of ``Obviousness,'' defined not as an epistemic invariant but as a representation-dependent cost signal: the inverse of the computational effort required to construct a proposition. While modern science and cognition are architected to manufacture obviousness to maximise efficiency, we demonstrate that this process creates a systematic ``epistemic posture'' that is inherently fragile. As a representation becomes more powerful, it makes more claims cheap to construct, thereby incentivising the collapse of verification budgets in exactly the regions where the representation is strongest. This rational blindness produces a characteristic error geometry: average performance improves while vulnerabilities concentrate in the uncompressed tails. We isolate this mechanism across three domains: discrete classification (Sparse Parity), continuous approximation (Runge's Boundary Divergence), and feature selection under distribution shift (Shortcut Selection), demonstrating that whenever obviousness is used as a proxy for safety, the tails become the natural hiding place of error.
\end{abstract}

\maketitle

% Section index (preferred). Migrate legacy sections below into manuscript/sections/.
\input{../sections/index.tex}

\section{Introduction}

A mature field does not only discover truths. It manufactures obviousness. 
Not in the cheap sense of ``everybody already knew,'' but in the precise 
sense that once a representation is built - notation, standard reductions, 
a preferred basis, a library of lemmata - whole regions of the world become 
cheap to traverse. 
...
Conflating those two - taking cheapness as a proxy for correctness - would 
merely 
...
Verification collapses exactly where the representation is strongest - 
because that is where construction is cheapest - and the remaining 
verification effort 
...
Average performance improves - often dramatically - while vulnerabilities 
concentrate 
...
As Polanyi noted, a mastered toolkit becomes part of our \textit{tacit 
dimension} - we 
...
representation - as if it were a property of the proposition in any 
representation, 
or even as if it had always been cheap. 
This is what has been termed the ``hindsight of insight'': the 
...
cheap to construct - shifting it from a high-cost state $C_{R_0}(P)$ in the 
old representation to a low-cost state $C_{R_1}(P)$ in the new one - 
observers fail 
...
and sparse structure. We demonstrate this error geometry in three settings: a discrete classification task with rare exceptions (Sparse Parity), a continuous approximation problem with boundary anomalies (Runge's Boundary Divergence), and a feature-selection task under distribution shift (Shortcut Selection).
...
\end{equation}
where $C_{cons}(P|R)$ is the computational or metabolic effort required to 
derive or instantiate $P$ given the primitives of $R$. This construction 
cost is distinct from the \textit{verification cost} $v(P)$ - the effort 
required to check $P$ against ground truth. The signal $O_R(P)$ provides the agent with a heuristic proxy for the reliability of $P$ without necessitating full verification. Following the logic of rational inattention \cite{sims2003implications}, a bounded agent must allocate a finite verification budget $V_{total}$ across a set of processed claims. The next subsection shows how low construction cost (high obviousness) drives reduced verification effort.

\paragraph{Operational Proxies for Construction Cost}
While $C_{cons}$ is theoretically grounded in Kolmogorov complexity, we 
operationalize it domain-specifically in our simulations:
\begin{itemize}
    \item \textbf{Decision Trees (Sparse Parity)}: $C_{cons}(P|R) \propto$ 
    incremental tree complexity to represent exception $P$ (additional 
    splits or depth required beyond baseline)
    
    \item \textbf{Polynomial Approximation (Runge)}: $C_{cons}(P|R) \propto$ 
    minimal polynomial degree required to approximate $P$ within tolerance 
    $\varepsilon$
    
    \item \textbf{Feature Selection (Shortcut)}: $C_{cons}(P|R) \propto$ 
    weighted feature count ($|w_i| \times \text{signal}_i$ for features 
    needed to represent $P$)
\end{itemize}
These are computable proxies that capture ``representational effort'' - 
claims requiring fewer or simpler primitives have lower $C_{cons}$.
...
\subsection{Epiplexity and the Compression of Structural Content}

To ground our framework in modern information theory for bounded observers, we 
connect to the recently introduced concept of \textit{Epiplexity} 
\cite{finzi2026epiplexity}. Classical information measures (Shannon entropy, 
Kolmogorov complexity) assume observers with unlimited computational capacity 
and treat all information equally. Epiplexity instead distinguishes between 
two types of content in data:

\begin{itemize}
    \item \textbf{Structural content} (epiplexity): Learnable patterns and 
    emergent structures that a computationally bounded observer can extract 
    and reuse for generalization
    
    \item \textbf{Time-bounded entropy}: Random, unpredictable content that 
    remains incompressible to the bounded observer (e.g., pseudorandom noise, 
    chaotic dynamics)
\end{itemize}

Epiplexity quantifies the length of the shortest program that captures the 
structural content under computational constraints, while time-bounded entropy 
measures the residual unpredictability.

\paragraph{Connection to Our Framework}
In our framework, representational progress $R_0 \to R_1$ is precisely the 
process of \textit{extracting structural content} from what was previously 
unpredictable. A representation $R$ is a compression that makes certain 
patterns learnable - it shifts content from time-bounded entropy into 
structural form. This creates two classes of claims:

\begin{itemize}
    \item \textbf{Compressed claims} $\mathcal{P}_{fluent}$: Those whose 
    structure has been extracted by $R$, making them cheap to construct 
    (low $C_{cons}$, high obviousness)
    
    \item \textbf{Uncompressed tail} $\mathcal{E}$: Claims whose structure 
    remains unextracted, requiring high construction cost despite the 
    representation
\end{itemize}

The Obviousness Trap arises because agents treat low construction cost 
(extracted structure) as evidence of reliability, allocating verification 
effort away from the compressed bulk. The tail $\mathcal{E}$ - where structure 
remains unextracted - becomes systematically unverified.
The ledger for this investment is up-front: paying for the toolkit, the basis, 
or the program bits. Once these bits are paid, the per-claim construction cost 
drops. The \textit{Obviousness Trap} is a structural bookkeeping failure: by 
treating $C_{cons}(P|R)$ as the total measure of reliability, agents ignore 
the sunk $L(M)$ required to make that claim cheap. Scrutiny collapses precisely 
where the investment has been most successful, leaving the uncompressed residue 
in the tails as the only remaining hiding place for error.
...
\paragraph{Formal Definition of the Tail}
We distinguish two notions of ``tail'':

\textbf{Ex ante tail} $\mathcal{E}_{gen}$: A subset specified by the 
environment or data-generating process (rare condition, boundary region, 
shifted distribution). This is defined \textit{before observing errors}. 
In our simulations:
\begin{itemize}
    \item Sparse Parity: $\mathcal{E}_{gen} = \{x : x_{N-1} = 1\}$ 
    (exception condition)
    \item Runge: $\mathcal{E}_{gen} = \{x : |x - 0.98| < 0.05\}$ 
    (boundary spike region)
    \item Shortcut: $\mathcal{E}_{gen} = D_{shift}$ 
    (shifted test distribution)
\end{itemize}

\textbf{Representational tail} $\mathcal{E}_{rep}$: A subset for which the 
representation update provides bounded cost reduction: 
$C_{cons}(P|R_1) - C_{cons}(P|R_0) \leq \delta$, where $\delta$ is a 
threshold (e.g., 10\% of mean cost drop across all claims). These are claims 
that remain ``uncompressed'' by the new representation.

Our simulations test the hypothesis: $\mathcal{E}_{gen} \approx 
\mathcal{E}_{rep}$. That is, do generator-defined tails coincide with 
representation-defined tails? This prediction is testable and non-circular: 
both tail sets are defined independently of error outcomes.
...
Under monotone ``obviousness $\to$ reduced verification'' policies, 
representational compression produces an error geometry in which bulk error 
decreases while tail-conditioned error (error restricted to $\mathcal{E}$) 
increases. This prediction is testable: if compression does not relocate 
error to the tail set, the theory fails.

\section{Results: The Hiding Place of Error}

Our simulations confirm that the manufacturing of obviousness leads to a 
specific error geometry. Three clarifications before presenting results. First, this is not a claim 
that ``obvious implies false'' - the bulk predictions remain accurate. 
Second, this does not require irrational agents: the mechanism arises under 
scarcity and monotone heuristics. Third, this is not a claim that 
representational progress universally worsens outcomes; rather, it relocates 
error unless verification adapts alongside compression. Fourth, the mechanism 
assumes construction fluency correlates with verification ease in the domain. 
This holds in most natural and scientific domains (claims easy to state tend 
to be easy to check) but fails in adversarial domains like cryptography, 
where construction is cheap (hash computation) but verification is expensive 
(preimage search). The trap occurs when agents mistakenly use construction 
cost as a proxy for verification cost in domains where this correlation is 
weak or absent. We claim it \textit{relocates} 
error to the tail set $\mathcal{E}$ unless verification policy adapts to 
compensate. The error does not vanish - it concentrates.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{../figures/fragility_curve.pdf}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{../figures/runge_fits.pdf}
    \vspace{0.2cm}
    \includegraphics[width=\columnwidth]{../figures/continuous_fragility.pdf}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\columnwidth]{../figures/shortcut_fragility.pdf}
...
A familiar psychological effect sits at the centre of this paper: once a 
representation is learned, fluency leaks backward 
\cite{fischhoff1975hindsight, ash2012hindsight}. We experience a claim as 
``obvious,'' misremember it as having been easy all along, and under-allocate 
scrutiny precisely where the current framework runs smoothly. In machine 
learning, the same geometry appears under different names - shortcut 
learning \cite{geirhos2020shortcut}, underspecification 
\cite{d'amour2022underspecification}, and brittle generalisation under 
shift - where models achieve high average 
performance by exploiting low-cost structure that evaluation fails to pressure-test. 
Our contribution is to treat these as instances of a single mechanism: 
representational compression lowers construction cost, and under a finite 
budget \cite{sims2003implications}, that cheapness is converted into a 
posture of diminished verification, pushing error into the uncompressed 
tails.

\paragraph{Disconfirmation Criterion}
The mechanism would be falsified if increasing representational compression 
(lowering bulk construction cost) did not systematically reduce verification 
allocation in the bulk, or if tail-conditioned error failed to rise under 
such reallocation. Our simulations confirm both predictions, but the theory 
remains testable in other domains.

\subsection{Structural investment and the erased ledger}

The shrinking verification budget is not an independent behavioural quirk. It is 
the downstream effect of structural investment in representation. When a field, 
or an agent, builds a new representation, it is not merely adopting a different 
vocabulary: it is reshaping the cost landscape. Propositions that previously sat 
at a high construction cost $C_{R_0}(P)$ are moved into a low-cost regime 
$C_{R_1}(P)$. This is productive novelty in its most concrete form - making 
previously prohibitive checks routine.

The cruelty is that this investment tends to erase its own evidence. Once 
$C_{R_1}(P)$ is in place, the cost of the compression step drops out of the 
ledger \cite{hasher1977frequency}. Cheapness is misattributed to the proposition 
itself (``it was always obvious''), and the representational work that produced 
the cheapness is reclassified as trivial or inevitable. This is not only a story 
about credit \cite{merton1968matthew}. It is also the mechanism by which the 
budget rule tightens: when the local experience of cheap traversal is treated 
as a proxy for safety, scrutiny collapses where the representation is most fluent.

\subsection{Construction cost and the standard toolkit}

In practice, the trap is strengthened by what might be called the \textit{standard 
toolkit effect}. Agents default to representations with negligible startup 
cost - canonical bases, standard approximations, inherited intuitions - 
because 
these are precompiled by culture, libraries, and habit. The result is that 
``obvious'' becomes synonymous with ``expressible in the toolkit.'' Discovering 
the representation required to expose an anomaly, by contrast, is itself 
expensive: it requires a search through representation space. Under scarcity, 
that search is discounted, and the agent settles for the basis that makes the 
bulk cheap, even if it makes the decisive exception effectively invisible.

This is precisely what our simulations illustrate in different guises. In 
Sparse Parity, compressive pressure makes the exception structure economically 
irrational to represent; error concentrates on the rare set. In Runge's 
Boundary Divergence, a smooth, low-degree representation is cheap and therefore 
preferred, but it must smear or ignore a narrow spike; increasing degree 
can recover local detail but also introduces global oscillatory artefacts without 
targeted representational choice. In both cases, the mechanism is not 
``poor learning,'' but cheap traversal driving reduced attention to what is 
expensive to represent.

\subsection{Connection to modern machine learning}

Read as machine learning, the \textit{Obviousness Trap} is the structural analogue 
of shortcut learning \cite{geirhos2020shortcut}. When a low-cost feature map 
captures the bulk of the label signal in a training distribution, optimisation 
will find it because it is cheap. What is expensive is the robust structure: 
invariances that reveal themselves only under shift, in rare subpopulations, or 
in adversarial regimes. Standard evaluation acts as a weak surrogate for 
verification: it pressure-tests performance primarily on typical draws from the 
training distribution. Under that regime, models become competent in the bulk 
and fragile in the corners.

The same geometry underlies underspecification \cite{d'amour2022underspecification}. 
Many internal hypotheses are equally consistent with the bulk of data; the 
evaluation environment fails to select among them; deployment reveals which 
distinctions were never tested. From the present viewpoint, this is not a 
separate pathology but a predictable consequence of treating representational 
cheapness as a proxy for safety: the regions where the representation compresses 
most efficiently become the regions where scrutiny is least informative and 
least applied.

\subsection{Hallucination as low-cost, non-verified structure}

The framework also gives a clean handle on hallucination. A hallucination is 
not best thought of as ``randomness,'' but as low-cost structure emitted 
without the cost of coupling to an external constraint. In a generative model, 
there is constant pressure to produce continuations that are cheap in the internal 
representation: fluent, coherent, strongly supported by priors. Fluency is 
exactly the cost signal identified by the theory. When external verification is 
expensive or absent - no grounded retrieval, no cross-check, no coupling to 
a second representation - cheap internal objects are emitted as if they 
were safe.

This explains why hallucinations are often well-formed. Coherence is cheap. 
What is expensive is fidelity to the contingent, high-friction details of the 
world. Under a budget rule that treats cheapness as a reason to stop checking, 
the expensive constraints are the first to be dropped. Hallucination is then not 
an anomaly of the model; it is the natural output of an obviousness engine 
run without a verification channel.

The \textit{mono-representation problem} follows as a consequence of single 
program class dominance. A mono-representational regime is one in which a 
single program class or representational framework dominates the 
construction and verification of claims, making obviousness self-certifying 
within that framework. When one representation defines what counts as 
efficient, scrutiny is applied only internally, as a check for 
self-consistency within the prevailing code. To escape this, we propose the 
structural antidote of \textit{coupling inequivalent representations}. 
Given two representations $R^{(1)}, R^{(2)}$, define a consistency penalty 
$K(P) = \text{dist}(\tau_{12}(P), P)$ under translation $\tau_{12}$; 
claims that are cheap in one lens but incur large $K$ are exactly the 
``unverified margin.'' By demanding \textit{mutual consistency} across 
structurally distinct program classes, an agent can force scrutiny back 
into the precise regions where any single compression would least like to 
pay it.

As we move toward more autonomous and compressed AI systems, the central 
challenge is no longer just the discovery of better models, but the 
management of the representational ledger. What is the true cost of a 
representation, where is it being paid - in compute, in data curation, in 
cognitive load, or in social infrastructure - and what objective governed the 
decision to buy that representation versus spending the equivalent effort on 
ongoing verification? These are the questions that will determine whether our 
intelligence becomes more capable, or simply more elegantly blind.
...
The central question remains: What is the true cost of a representation, where is it being paid - in compute, in data curation, in 
cognitive load, or in social infrastructure - and what objective should 
govern the decision to invest in new representational toolkits versus spending equivalent effort on ongoing verification? These are the questions that will determine whether our intelligence becomes more capable, or simply more elegantly blind.
